...
const TemplateThemeConfigurator = ({ template, value, onChange }) => {
  ...
  const densityOptions = [
    { value: 'compact', label: tt('templateThemeConfigurator.density.compact', '紧凑') },
    { value: 'comfortable', label: tt('templateThemeConfigurator.density.comfortable', '标准') },
    { value: 'spacious', label: tt('templateThemeConfigurator.density.spacious', '舒展') },
  ]
  const fontOptions = [
    { value: '"Segoe UI", "PingFang SC", sans-serif', label: tt('templateThemeConfigurator.fonts.modernSans', '现代无衬线') },
    { value: '"Microsoft YaHei", "PingFang SC", sans-serif', label: tt('templateThemeConfigurator.fonts.yaheiChinese', '雅黑中文') },
    { value: '"Georgia", "Times New Roman", serif', label: tt('templateThemeConfigurator.fonts.classicSerif', '经典衬线') },
  ]
  ...
  return (
    <div className='rounded-3xl border border-slate-200 bg-white p-5 space-y-4'>
      <div>
        <div className='text-sm font-semibold text-slate-800'>{tt('templateThemeConfigurator.title', '主题参数')}</div>
        <p className='mt-1 text-xs text-slate-500'>{tt('templateThemeConfigurator.description', '当前模板支持颜色、字体与版式密度配置，后续社区模板会复用这套参数协议。')}</p>
      </div>

      {presets.length > 0 && (
        <div>
          <div className='text-xs font-semibold text-slate-600 mb-2'>{tt('templateThemeConfigurator.presets', '预设主题')}</div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
        <label className='text-sm text-slate-700'>
          <div className='mb-2 font-semibold'>{tt('templateThemeConfigurator.fields.accentColor', '强调色')}</div>
        ...
        <label className='text-sm text-slate-700'>
          <div className='mb-2 font-semibold'>{tt('templateThemeConfigurator.fields.headingColor', '标题色')}</div>
        ...
        <label className='text-sm text-slate-700'>
          <div className='mb-2 font-semibold'>{tt('templateThemeConfigurator.fields.tagBackground', '标签底色')}</div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
        <label className='text-sm text-slate-700'>
          <div className='mb-2 font-semibold'>{tt('templateThemeConfigurator.fields.fontFamily', '字体方案')}</div>
        ...
        <label className='text-sm text-slate-700'>
          <div className='mb-2 font-semibold'>{tt('templateThemeConfigurator.fields.density', '信息密度')}</div>
